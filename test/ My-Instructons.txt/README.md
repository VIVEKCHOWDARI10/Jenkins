...
2. This way is much simpler ,go to credentials add the github token using username and password and then  go to pipeline script and then click on pipeline script and then add github token there so it will give you the script ,, done paste the script ( no ssh required ) 
